...
For example when deploying a cluster for a stage environment, it's a good practice to namespace all the secrets used by the applications in that environment with `/stage/`, so an application could store its secrets in `/stage/my-application/secrets`. In this case the `secrets_paths` variable should be set to `secrets_path = "/stage/*"`. If you need to add multiple prefix values, you can add an array, e.g. `["/stage1/*", "/stage2/*"]`. All other values in `external_secrets_settings` are passed to the external_secrets helm chart as Values, so should generally have a string value.
